...
\def\title{Möglichkeiten der Implementierung und Nutzung von RESTful APIs in Anwendungsarchitekturen auf IBM z/OS (Mainframe)}		% Titel der Arbeit
\def\titleFrontpage{Möglichkeiten der Implementierung und Nutzung \\von RESTful APIs in Anwendungsarchitekturen \\auf IBM z/OS (Mainframe)}
\def\doctype{Projektarbeit 1 (T2\_1000)}	% Art des Dokumentes, z.B. Studienarbeit, Bachelorarbeit, Bericht, ...
...
\def\bearbeitungsZeit{10 Wochen}			% Bearbeitsungszeitraum
\def\ausbildungsfirma{Deutsche Bundesbank} 	% Ausbildungsfirma
\def\betreuerFirma{Eric Zenker}				% Betreuer der Ausbildungsfirma
%\def\betreuerDH{Titel Vorname Nachname}		% Betreuer der Dualen Hochschule
\def\abgabeDatum{\today}					% Abgabedatum der Arbeit
% Späteste Abgabe bei Uni: 09.09.2016 (Ende 1. Woche des 3. Semesters)
